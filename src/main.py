...
from src.utils.training_helper_MixTS import main_wrapper_MixTS
...
def parse_args():
    ...
    parser.add_argument('--model',choices=['co_teaching','co_teaching_mloss',
                                           ...
                                           'single_ae_aug_sel_allaug','single_ae_aug_before_sel','dividemix', 'CTW','MixTS'],
                        default='CTW')
    ...
    parser.add_argument('--L_rec_coef', type=float, default=0.,
    ...
    parser.add_argument('--L_rea', type=float, default=0.5, help='if use queue for sup-cl')
    parser.add_argument('--L_match', type=float, default=1, help='if use queue for uns-cl')
    parser.add_argument('--k_val', type=int, default=10, help='k for k-nn correction')
    parser.add_argument('--sup_t', default=1, type=float, help='sup-cl temperature')
    parser.add_argument('--th_gama', type=float, default=0.5, help='example selection th')
    ...
    parser.add_argument('--batch_mu', type=float, default=1, help='batch_mu ')
    parser.add_argument('--pn_strength', default=0.1, type=float)
    parser.add_argument('--use_threshold_examples', default=True, type=bool)
    ...
    parser.add_argument('--threshold_type', default=0., type=float,
                        help='threshold calculation type'
                             '-1: mean threshold(like CTW)'
                             '0: margin threshold'
                             '0-1: fixed threshold')
...
def main(args, dataset_name=None):
    ...
    skf = StratifiedKFold(n_splits=len(args.manual_seeds))
    id_acc = 0
    seeds_i = -1
...
if __name__ == '__main__':
    ...
    # Logging setting
    if args.debug:  # if not debug, no log.
